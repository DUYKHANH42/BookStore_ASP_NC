...
namespace BookStore.API.Areas.Admin.Controllers
{
    ...
    public class OrderController : Controller
    {
        ...
        public async Task<IActionResult> UpdateStatus(int id, string status)
        {
            ...
            var oldOrder = await _orderService.GetOrderDetailsAsync(id);
            var result = await _orderService.UpdateOrderStatusAsync(id, status, adminName);
            if (result)
            {
                var oldStatus = oldOrder?.Status ?? "Không rõ";
                await _activityLog.LogAsync(
                    ActivityModules.Order,
                    ActivityActions.StatusChange,
                    $"Đổi trạng thái đơn hàng #{id} từ '{oldStatus}' sang '{status}'",
                    entityType: "Order",
        ...
        [HttpPost]
        public async Task<IActionResult> CreatePOS([FromBody] POSOrderDTO request)
        {
            var adminName = User.Identity?.Name ?? "Admin";
            var result = await _orderService.CreatePOSOrderAsync(request, adminName);
            if (result.Success)
            {
                await _activityLog.LogAsync(ActivityModules.Order, ActivityActions.Create, result.ActivityDetails, entityType: "Order", entityId: result.OrderId.ToString());
                return Json(new { success = true, orderId = result.OrderId, message = "Tạo đơn hàng thành công" });
            }
