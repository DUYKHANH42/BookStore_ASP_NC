...
namespace BookStore.Infrastructure.Repositories
{
    public class CartRepository : GenericRepository<Cart>, ICartRepository
    {
        ...
        {
            var vnNow = TimeHelper.GetVnTime();

            return await _context.Carts
                .Include(c => c.Items)
                    .ThenInclude(i => i.Product)
                        .ThenInclude(p => p.FlashSales
                            .Where(fs => fs.FlashSaleCampaign.IsActive
                                         && vnNow >= fs.FlashSaleCampaign.StartTime
                                         && vnNow <= fs.FlashSaleCampaign.EndTime
                                         && fs.SoldCount < fs.SaleStock))
                            .ThenInclude(fs => fs.FlashSaleCampaign) 
